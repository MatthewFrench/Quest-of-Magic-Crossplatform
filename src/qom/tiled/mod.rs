...
impl Map {
    fn new<R: Read>(
        ...
        existing_tilesets: &HashMap<String, String>,
        ...
        parse_tag!(parser, "map", {
            "tileset" => | attrs| {
                tilesets.push(Tileset::new(parser, attrs, map_path, existing_tilesets)?);
                Ok(())
            },
            "layer" => |attrs| {
...
impl Tileset {
    fn new<R: Read>(
        ...
        Tileset::new_internal(parser, &attrs)
            .or_else(|_| Tileset::new_reference(&attrs, map_path, existing_tilesets))
    }
    ...
    fn new_reference(
        ...
        if existing_tilesets.contains_key(&source) {
            return Tileset::new_external(
                existing_tilesets.get(&source).unwrap().as_bytes(),
                first_gid,
            );
        }
...
fn parse_impl<R: Read>(
    reader: R,
    map_path: Option<&Path>,
    tilesets: &HashMap<String, String>,
) -> Result<Map, TiledError> {
        ...
        match parser.next().map_err(TiledError::XmlDecodingError)? {
            XmlEvent::StartElement {
                ...
                if name.local_name == "map" {
                    return Map::new(&mut parser, attributes, map_path, tilesets);
                }
            }
            XmlEvent::EndDocument => {
                return Err(TiledError::PrematureEnd(
...
pub fn parse_with_path<R: Read>(reader: R, path: &Path) -> Result<Map, TiledError> {
    parse_impl(reader, Some(path), &HashMap::new())
}
...
pub fn parse_file(path: &Path) -> Result<Map, TiledError> {
    ...
    parse_impl(file, Some(path), &HashMap::new())
}
...
pub fn parse<R: Read>(
    ...
    parse_impl(reader, None, existing_files)
}
